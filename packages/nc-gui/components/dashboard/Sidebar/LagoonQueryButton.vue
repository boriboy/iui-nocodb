...
<template>
  <div v-if="base?.id || resolvedProject?.id" class="flex flex-col gap-1">
    <NcSidebarMenuItem
      ...
      data-testid="nc-lagoon-query-launcher"
      @click="openLagoonQuery()"
    >
      <template #icon>
        <GeneralIcon icon="ncTable" class="flex-none h-5 w-5" />
      </template>
      <span>Lagoon Quadrats</span>
    </NcSidebarMenuItem>
  </div>
</template>
